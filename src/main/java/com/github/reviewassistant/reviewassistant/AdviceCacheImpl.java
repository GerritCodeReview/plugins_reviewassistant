...
/**
 * This implementation of AdviceCache writes to the plugin's data directory ({gerrit
 * url}/plugins/ReviewAssistant). The structure follows that of git's object directory, which means
 * that the first two letters of the commit's SHA-1 is used as name for the sub directory, and the
 * rest of the SHA-1 is used as file name.
 */
public class AdviceCacheImpl implements AdviceCache {
  ...
  private File getCalculationFile(String revision) {
    return new File(dir, revision.substring(0, 2) + File.separator + revision.substring(2));
  }
    ...
    if (calculation == null || calculation.totalReviewTime == 0) {
      ...
      try {
        ...
        double reviewTimeModifier =
            cfg.getProjectPluginConfigWithInheritance(resource.getChange().getProject(), pluginName)
